//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace Models
{
    using App_Code;
    using System;
    using System.Collections.Generic;
    using System.Data;
    using System.Data.SqlClient;
    public partial class clsPublication_NonISI_Author
    {
        #region Fields
        public static string Publication_NonISI_Author_ID = "Publication_NonISI_Author_ID";
        public static string KFUPMID = "KFUPMID";
        public static string Publication_NonISI_ID = "Publication_NonISI_ID";
        public static string isCorresponding_Author = "isCorresponding_Author";
        public static string Remarks = "Remarks";
        public static string CreatedOn = "CreatedOn";
        public static string CreatedBy = "CreatedBy";
        public static string UpdatedOn = "UpdatedOn";
        public static string UpdatedBy = "UpdatedBy";
        public static string Active_flag = "Active_flag";
        public static string Deleted_flag = "Deleted_flag";
        #endregion
    }

    public partial class Publication_NonISI_AuthorController
    {
        #region Fields
        public int Publication_NonISI_Author_ID { get; set; }
        public double? KFUPMID { get; set; }
        public int? Publication_NonISI_ID { get; set; }
        public bool? isCorresponding_Author { get; set; }
        public string Remarks { get; set; }
        public DateTime CreatedOn { get; set; }
        public string CreatedBy { get; set; }
        public DateTime? UpdatedOn { get; set; }
        public string UpdatedBy { get; set; }
        public bool? Active_flag { get; set; }
        public bool? Deleted_flag { get; set; }
        #endregion

        #region Methods

        /// <summary>
        /// Adds an Author entry for a Non-ISI publication
        /// </summary>
        /// <param name="kfupmid">Researcher's KFUPM ID</param>
        /// <param name="publicationid">Non-ISI Publication ID</param>
        public static void AddNonISIPublicationAuthor(int kfupmid, int publicationid)
        {
            string strStoredProcedure = "sp_rep_AddAuthorToNonISIPublication";

            #region Add parameters

            List<SqlParameter> param = new List<SqlParameter>();
            param.Add(new SqlParameter("@kfupmid", kfupmid));
            param.Add(new SqlParameter("@pubid", publicationid));

            #endregion

            DbAccess.ExecuteQuery(strStoredProcedure, CommandType.StoredProcedure);

        }

        #endregion
    }
}
